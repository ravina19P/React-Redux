...
import { DeleteProduct, GetMonitorData, commonAPi } from '../../Other/Common';
import { useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { storemonitorData } from '../../Redux/react-redux/action';
...
const Monitor = () => {
    ...
    useEffect(() => {
        ...
        setAdmin(isAdmin)
        context.setActiveTB("Monitor")
        if (ReducerData.monitorData.length === 0) {
            GetMonitorData().then((data) => {
                dispatch(storemonitorData(data))
            });
            GetMonitorData();
        }
    }, [])
    ...
    return (
        <div className={`container mt-2 ${style.backgroundchange}`}>
            <h1>Monitor</h1>
            <div className='row'>
                {ReducerData.monitorData.length > 0 && ReducerData.monitorData.map((item, index) => {
                    return (
